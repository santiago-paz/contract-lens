...
/*   contractPartner: z.string().nullable().describe('The counterparty or partner organization'),
  ...
  contractStart: z.union([
    z.string().regex(/^\d{4}-\d{2}-\d{2}$/),
    z.string() // Allow other strings like "Not specified" or "undefined" to avoid validation errors
  ]).nullable().describe('Start date in ISO YYYY-MM-DD format, or "Not specified" if not found'),
  
  expirationDate: z.union([
    z.string().regex(/^\d{4}-\d{2}-\d{2}$/),
    z.string() // Allow other strings like "Not specified" or "undefined" to avoid validation errors
  ]).nullable().describe('The specific end date or expiration date in ISO YYYY-MM-DD format, or "Not specified" if not found'),
  durationType: z.union([
    z.enum(['One-time', 'Fixed-term', 'Indefinite']),
    z.string()
  ]).nullable().describe("How the duration is defined in the text."),
  ...
  riskAssessment: z.union([
    z.enum(['Low', 'Medium', 'High']),
    z.string()
  ]).nullable().describe('Overall risk level based on clauses and liability'), */
