...
export default async function AllExpiringContractsPage({
  searchParams,
}: {
  ...
  else filtered = withDays.filter((c) => c.daysRemaining <= 90);
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-black text-black uppercase tracking-tight">All Expiring Contracts</h1>
        <p className="text-gray-500 text-sm mt-1 uppercase font-medium">All contracts across your organization approaching expiration</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {summaryCards.map((card) => (
          <Link
            key={card.key}
            href={`/all-expiring?urgency=${card.key}${q ? `&q=${q}` : ''}`}
      ...
      {/* Search */}
      <AllExpiringSearch defaultValue={q} urgency={urgency} />
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  ...
                  <th className="px-6 py-3 text-[10px] font-bold text-gray-500 uppercase tracking-wider">Owner</th>
          ...
          <div className="flex flex-col items-center justify-center p-16 text-center">
            ...
            {q || urgency !== 'all' ? (
              ...
              <>
                ...
                <p className="text-gray-500 max-w-xs text-xs">No contracts in the organization have an end date set, or none are expiring within 90 days.</p>
              </>
            )}
          </div>
        )}
...
import { AllExpiringSearch } from './AllExpiringSearch';
